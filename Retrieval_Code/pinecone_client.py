...
def retrieve_and_rerank(query: str, namespace: str, metadata_filter: dict = None,
                        top_k: int = 40, top_n = 10) -> str:
    ...
    if len(docs) <= 1 or top_n < 1:
        return "\n".join(f"{i+1}. {m.metadata.get('text','')}"
    ...
    # 4. Build the formatted output string
    chunks = []
    for i, hit in enumerate(reranked.data, start=1):
        txt = hit.document.text
